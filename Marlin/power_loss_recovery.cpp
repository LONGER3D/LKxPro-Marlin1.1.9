  ...
  if (card.cardOK) {
    ...
    if (card.jobRecoverFileExists()) {
      ...
      if ((job_recovery_info.valid_head!=0)&&(job_recovery_info.valid_head == job_recovery_info.valid_foot)) {
		...
		char str_Z[16], str_E[16]/*,str_Z_fade[16],str_E_up[16]*/;
		memset(str_Z, 0, sizeof(str_Z));
		memset(str_E, 0, sizeof(str_E));
//		memset(str_Z_fade,0,sizeof(str_Z_fade));
//		memset(str_E_up, 0, sizeof(str_E_up));

		dtostrf(job_recovery_info.save_current_Z, 1, 3, str_Z); //Z
		dtostrf(job_recovery_info.save_current_E, 1, 3, str_E);  //E    -4  -5
...
void save_job_recovery_info() {
  ...
  if ((current_position[2] > 0 &&abs(( current_position[2]+ recovery_z_height) -job_recovery_info.save_current_Z)>=0.1)) 
  {
   ...
   // COPY(job_recovery_info.current_position, current_position);
	...
	//for (i = 0; i < 2; i++)
	//{
	//	job_recovery_info.current_position[i] = current_position[i];
	//}
	job_recovery_info.save_current_Z = current_position[2]+recovery_z_height;
	job_recovery_info.save_current_E = current_position[3]/*+ recovery_E_len*/;
	//MYSERIAL0.println("LGT");
	//MYSERIAL0.println(current_position[2]);
	//MYSERIAL0.println(job_recovery_info.current_position[2]);
